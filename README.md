...
四柱八字排盘与通俗命理解读，Codex Skill，自然化语言，优雅的网页版HTML，
...
```text
https://github.com/CCBrother/shengchenbazi/tree/main/bazi-reading
```

### 第二步：配置 DeepSeek API

前往 [DeepSeek 开放平台](https://platform.deepseek.com/) 注册账号，创建 API Key，并充值少量余额。一次命理解读通常只需几分钱，实际费用会根据模型价格、输入内容和输出长度变化。# 生辰八字命理解读 Skill
